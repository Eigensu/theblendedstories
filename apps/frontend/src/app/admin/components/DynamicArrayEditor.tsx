...
type ArrayEditorProps = {
  ...
  fields: {
    ...
    guidelineKey?: import('../../../constants/imageGuidelines').GuidelineKey;
  }[];
  defaultNewItem: any;
};

export default function DynamicArrayEditor({
  endpoint,
  itemTitleField,
  fields,
  defaultNewItem,
    ...
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="h-24 bg-[#111111] border border-zinc-800 rounded-2xl animate-pulse"
          ></div>
        ))}
      </div>
    );
      ...
      {items.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-16 bg-[#111111] rounded-2xl border border-dashed border-zinc-800 shadow-sm">
          ...
          <p className="text-sm text-zinc-500 mb-6 text-center max-w-sm">
            Create your first item to populate this section.
          </p>
          <button
            onClick={handleCreate}
                      ...
                      {(provided, snapshot) => (
                        <div
                          ...
                          className={cn(
                            ...
                            snapshot.isDragging
                              ? 'shadow-xl border-white scale-[1.02] z-50'
                              : 'shadow-sm border-zinc-800 hover:border-zinc-700',
                            !isVisible && 'opacity-75 bg-black'
                          )}
                        >
                          {/* Not a <button>: the drag handle below is itself
                            interactive, and nesting interactive elements is
                            invalid. role + tabIndex + onKeyDown gives the same
                            keyboard behaviour without restructuring the row. */}
                          <div
                            role="button"
                            tabIndex={0}
