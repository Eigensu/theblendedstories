...
export default function AdminDashboard() {
  ...
  const activeSection =
    adminSections.find((s) => s.id === activeSectionId) || adminSections[0];
  ...
  const getBreadcrumb = () => {
    ...
    if (activeSection.group === 'home')
      return `Dashboard / Pages / Home / ${activeSection.title}`;
    if (activeSection.group === 'crm')
      return `Dashboard / CRM / ${activeSection.title}`;
        ...
        >
          {isActive && isSidebarOpen && (
            <motion.div
              ...
              className="absolute left-3 w-1 h-5 bg-white rounded-full"
            />
          )}
          <span
            className={`mr-3 opacity-70 ${!isSidebarOpen && !indented && 'mx-auto'}`}
          >
          ...
          <nav className="flex-1 overflow-y-auto py-6 custom-scrollbar">
            <ul className="space-y-1 px-3">
                  ...
                  {homeOpen && isSidebarOpen && (
                    ...
                    >
                      {homeSections.map((section) =>
                        renderSidebarItem(section, true)
                      )}
                    </motion.ul>
                  )}
                </AnimatePresence>
              ...
              {standaloneSections.map((section) =>
                renderSidebarItem(section, false)
              )}
              ...
              {/* ── CRM Sections (Members, Subscribers, Waitlist) ── */}
              <li>
                <div className="px-3 py-2 text-xs font-semibold text-zinc-500 uppercase tracking-wider">
                  {isSidebarOpen ? (
                    'CRM'
              ...
              {crmSections.map((section) => renderSidebarItem(section, false))}
            </ul>
          </nav>
        ...
        <header className="bg-[#0A0A0A]/90 backdrop-blur-md border-b border-zinc-800/50 h-16 flex items-center px-8 justify-between flex-shrink-0 z-10 sticky top-0">
          <div className="flex items-center gap-4">
              ...
              {getBreadcrumb()
                ...
                .map((part, i, arr) => (
                  <React.Fragment key={i}>
                    <span
                      className={
                        i === arr.length - 1 ? 'text-white font-semibold' : ''
                      }
                    >
                      {part}
                    </span>
            ...
            {activeSectionId !== 'media_library' &&
              activeSection.group !== 'crm' && <StatusBadge status={status} />}
          </div>
                    ...
                    className={`relative flex items-center justify-center px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 ${
                      isSaving
                        ...
                        : hasUnsavedChanges
                          ...
                          : 'bg-black border border-zinc-800 text-zinc-600'
                    }`}
                  >
                    {isSaving && (
            ...
            {activeSectionId === 'media_library' ? (
              <div className="flex flex-col items-center justify-center p-16 bg-[#111111] rounded-2xl border border-dashed border-zinc-800 text-center">
                ...
                <p className="text-zinc-400 max-w-md">
                  Global media management is coming soon. For now, you can
                  upload media directly within each section editor.
                </p>
              </div>
            ) : (
